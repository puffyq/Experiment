...
class xuan:
    def __init__(self, K = 150, Lambda = 0.01, Eta = 60):
        self.data = None
        self.train = None
        self.test = None
...
#            betahat = np.zeros(self.K_factors)
    ...
    def regression(self,adj_matrix, y, x, r, flag):
        ...
        if (flag == 'p')|(flag == 's'):
            X = np.mat(np.dot(np.transpose(x), x) + np.eye(np.shape(x)[1]) * (self.Lambda + self.Eta)) 
#            k = np.linalg.matrix_rank(X)
            mid_1 = self.Eta * np.dot(np.transpose(adj_matrix), y) 
            mid_2 = np.dot(np.dot(np.transpose(adj_matrix),r),x)
#            mid_3 = np.dot(np.dot(np.transpose(adj_matrix), group_matrix),np.dot(np.transpose(x), x) )
#            betahat = np.dot(np.dot(np.linalg.pinv(np.dot(np.transpose(adj_matrix),adj_matrix)),(mid_1+mid_2-mid_3)),np.linalg.pinv(X))
        else:
            X = np.mat(np.dot(np.transpose(x), x) + np.eye(np.shape(x)[1]) * (self.Lambda + self.Eta)) 
#            k = np.linalg.matrix_rank(X)
            mid_1 = self.Eta * np.dot(np.transpose(adj_matrix), y) 
            mid_2 = np.dot(np.dot(np.transpose(adj_matrix),np.transpose(r)),x)
#            mid_3 = np.dot(np.dot(np.transpose(adj_matrix), group_matrix),np.dot(np.transpose(x), x) )

        betahat = np.dot(np.dot(np.linalg.pinv(np.dot(np.transpose(adj_matrix),adj_matrix)),(mid_1+mid_2)),np.linalg.pinv(X))
...
#X_USER = (P+S)
            ...
            #控制ALS一次迭代中backfitting的损失
                ...
                while beta_diff > 1e-3:
                    ...
                    betahat = self.regression(adj_q, V2_1, ps, link_new, 'q')
...
#更新Q只需要使（P+S）Q的损失最小，因为（P+S）(T)这一项相当于常数
                ...
                ps = V1_1 + V3_1[sc-1]
#                link_new = link.copy()
                link_new[index] =(link- np.dot(ps,np.transpose(V2_1)))[index]
                beta_init = np.array(np.random.normal(loc=0, scale=0.3, size=(12, self.K_factors)), dtype='float64')
                beta_diff = 2
                ...
                while beta_diff > 1e-3:
                    ...
                    betahat = self.regression(adj_t[tc-1], V4_1[tc-1], ps, link_new, 't')
...
#                print(beta_loss)                
            ...
            #控制ALS一次迭代中backfitting的损失
                ...
                qt = V2_1 + V4_1[tc-1]
                link_new = link.copy()
                link_new[index] =(link- np.dot(V3_1[sc-1],np.transpose(qt)))[index]
                beta_init = np.array(np.random.normal(loc=0, scale=0.3, size=(12, self.K_factors)), dtype='float64')
                beta_diff = 2
                ...
                while beta_diff > 1e-3:
                    ...
                    betahat = self.regression(adj_p, V1_1, qt, link_new, 'p')
...
#                print(beta_loss)                
                ...
                qt = V2_1 + V4_1[tc-1]
                link_new = link.copy()
                link_new[index] =(link - np.dot(V1_1,np.transpose(qt)))[index]
                beta_init = np.array(np.random.normal(loc=0, scale=0.3, size=(12, self.K_factors)), dtype='float64')
                beta_diff = 2
                ...
                while beta_diff > 1e-3:
                    ...
                    betahat = self.regression(adj_s[sc-1], V3_1[sc-1], qt, link_new, 's')
