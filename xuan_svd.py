...
class xuan_svd:
    def __init__(self, K = 150, Lambda = 0.01):
        self.data = None
        self.train = None
        self.test = None
...
#P是用户名向量；n是用户数
    ...
    def myridge(self,y, x):
        if len(y) == 0:
            betahat = np.zeros(self.K_factors)
        else:
            X = np.mat(np.dot(np.transpose(x), x) + np.eye(np.shape(x)[1]) * self.Lambda)
            k = np.linalg.matrix_rank(X)

            if k < np.shape(X)[0]:
                betahat = np.dot(np.linalg.pinv(X), np.dot(np.transpose(x), y))
            else:
                betahat = np.dot(X.I, np.dot(np.transpose(x), y))
...
#group里面要存储每个样本对中用户和朋友的分组信息
    ...
    def iterate(self,length, number, train, test, max_iter, lr=0.05, alg='LSE', cluster='ORI', clusters_num=10, ufile=None, ifile=None):
 #加入组间效应的SVD
...
#X_USER = (P+S)
            ...
            #控制ALS一次迭代中backfitting的损失
                ...
                for i in range(1, self.item_count + 1):
                    if alg == 'LSE':
                        V2_1[i - 1] = self.myridge(y2_bag[i], x2_bag[i])
                    elif alg == 'GD':
...
#更新Q只需要使（P+S）Q的损失最小，因为（P+S）(T)这一项相当于常数
                ...
                for i in range(1, self.igroup_count + 1):
                    if alg == 'LSE':
                        V4_1[i - 1] = self.myridge(y4_bag[i], x4_bag[i])
                    elif alg == 'GD':
...
#这个diff的要求不在于控制loss，而在于迭代稳定了，不太变化了，信息提取全面了
            ...
            #控制ALS一次迭代中backfitting的损失
                ...
                for i in range(1, self.user_count + 1):
                    if alg == 'LSE':
                        V1_1[i - 1] = self.myridge(y1_bag[i], x1_bag[i])
                    elif alg == 'GD':
                ...
                for i in range(1, self.ugroup_count + 1):
                    if alg == 'LSE':
                        V3_1[i - 1] = self.myridge(y3_bag[i], x3_bag[i])
                    elif alg == 'GD':
...
#        length = len(set(test[0][~np.isin(test[2],0.5)]))               
        recall,precision,recall_base,precision_base,f1,f1_base = top_k(self.data, test, cn_all, self.user_group, self.item_group, U1_init,U2_init,U3_init,U4_init, k=length) #int(0.3*number)     #                      
        auc_value = auc(test, y_hat) 
        
        evaluate_matrix={'auc_value':auc_value,'precision':precision,'recall':recall,'f1_gssvd':f1}
